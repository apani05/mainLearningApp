...
class CategoryItemState extends ConsumerState<CategoryItem> {
  @override
  Widget build(BuildContext context) {
    final vocaProvide = ref.watch(vocaProvider);
    return GestureDetector(
      onTap: () {
        print("title is ${widget.title}");
        vocaProvide.titleId = widget.title;
        Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => LearningPage(seriesName: widget.title)),
        );
      },
      child: Container(
        height: 60,
        ...
        child: ListTile(
          ...
          title: displaySeriesName(seriesId: widget.title, returnText: true),
        ),
      ),
    );
  }
